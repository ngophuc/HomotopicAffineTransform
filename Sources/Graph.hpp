...
//Code from : https://www.codeproject.com/Articles/1158232/Enumerating-All-Cycles-in-an-Undirected-Graph
namespace graph
{
	...
	public:
			...
			while (nodeStack.size() > 0)
			{
				...
				for (size_t j = 0; j < m_aNodes.size(); ++j)
				{
					...
					if (aTree[j].parent != &aTree[j])
					{
						...
						pi.dump(std::cerr);
						pj.dump(std::cerr);
						m_aFundamentalCycles.back().dump(std::cerr);
#endif
					}
					else
					{
        ...
        // Returns all computed cycles
        inline const CycleArray& getAllCycles() const { return m_aCycles; }
...
#ifdef __GRAPH_DEBUG__
				// Debug-Case: Validate cycle; should always be true as the fundamental cycles contain only one single fully connected cycle!
				os << (validateCycleMatrix(cycleMatrix) ? " valid!" : " invalid!");
#endif
				os << "\n";
			}
	...
	inline bool CreateRandomGraphIsConnected(const size_t& a, const size_t& b, const void* pEdgeProbability)
	{
		const float* pfProbability = reinterpret_cast<const float*>(pEdgeProbability);
		return static_cast<int>(*pfProbability * RAND_MAX) < rand();
	}

	std::unique_ptr<Graph<size_t> > CreateRandomGraph(size_t nNodes, float fEdgeProbability)
	{
		Graph<size_t>::NodeArray aNodes;
		for(size_t i = 0; i < nNodes; ++i)
			aNodes.push_back(i);
		return std::unique_ptr<Graph<size_t> >(new Graph<size_t>(aNodes, CreateRandomGraphIsConnected, &fEdgeProbability));
	}
}
...
int main()
{
    ...
    // This is the graph which is illustrated in Fig. 1a in the main text.
    ...
    // Version3:
    //        - This graph is added for testing of a larger graph
    cout << "\n\n==================================================\n"
          << "Enumerating cycles of a slightly larger graph:\n";

    // Another graph is tested which has 4 fundamental cycles and therefore can potentially have 11 cycles besides the fundamental ones.
    char aObjects2[] = { 'A', 'B', 'C', 'D', 'E', 'F', 'G' };
    size_t aEdges2[] = { 0,1, 0,2, 1,3, 1,4, 2,5, 3,4, 3,5, 4,5, 5,6, 3,6 };

    graph::Graph<char> g2(aObjects2, 7, aEdges2, 10);
    g2.computeFundamentalCycles();
    g2.computeAllCycles();
    g2.dump(cout);

    // Now the cycles are enumerated using the cycle iterator:
    cout << "\n\n==================================================\n"
         << "Enumerating cycles using the Cycle Iterator:\n"
         << "Note: The iterator enumerates _all_ cycles including the fundamental cycles.\n";

    graph::Graph<char>::CycleIterator it(&g);
    for (; it != graph::Graph<char>::CycleIterator::End(); ++it)
    {
        bool bFirst = true;
        graph::Graph<char>::NodePath path = g.cycleMatrix2nodePath(*it);
        for (const char* obj : path)
        {
            if (bFirst)
                cout << *obj;
            else
    ...
    // Create a large random graph with 40% Edge connection probability;
    // This should yield a graph with approximately 36 edges and it can produce around 20 fundamental
    // cycles, therefore do not compute all the cycles as it will take a while!
    cout << "\n\n==================================================\n";
    cout << "\nEnumerating Cycles of a random graph:\n";

    auto pGraph = graph::CreateRandomGraph(10, 0.4f);
    pGraph->computeFundamentalCycles();

    pGraph->dump(cout);
    // This might take a long time....
    //pGraph->computeAllCycles();
...
Graph contains 4 Nodes and 4 Edges
Printing 1 fundamental cycles:
A-B-C-D-A
Printing ALL 1 cycles:
A-B-C-D-A


==================================================
Graph contains 6 Nodes and 8 Edges
Printing 3 fundamental cycles:
A-B-E-F-C-A
D-E-F-D
A-B-D-F-C-A
Printing ALL 7 cycles:
A-B-E-F-C-A
D-E-F-D
A-B-D-F-C-A
A-B-E-D-F-C-A
...
==================================================
Enumerating cycles of a slightly larger graph:
Graph contains 7 Nodes and 10 Edges
Printing 4 fundamental cycles:
D-F-G-D
A-B-E-F-C-A
D-E-F-D
A-B-D-F-C-A
Printing ALL 12 cycles:
D-F-G-D
A-B-E-F-C-A
D-E-F-D
A-B-D-F-C-A
...
Enumerating Cycles of a random graph:
Graph contains 10 Nodes and 29 Edges
Printing 20 fundamental cycles:
0-6-9-0
0-7-9-0
0-8-9-0
1-5-9-1
